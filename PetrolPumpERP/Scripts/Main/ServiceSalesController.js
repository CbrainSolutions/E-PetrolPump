PetroliumApp.controller("ServiceSalesController", ['$scope', '$http', '$filter', '$rootScope', function ($scope, $http, $filter, $rootScope) {
    $scope.MainSalesList = [];
    $scope.SalesList = [];
    $scope.BankList = [];
    $scope.CustomerList = [];
    ...
    function GetSalesInvoices() {
        ...
        var url = GetVirtualDirectory() + "/ServiceSales/GetSalesInvoices";
        $http.get(url)
    ...
    $scope.Save = function (IsEdit) {
        if ($scope.ValidateForm()) {
            ...
            var url = GetVirtualDirectory() + '/ServiceSales/Save';
            if (IsEdit == false) {
                url = GetVirtualDirectory() + '/ServiceSales/Update';
            }
            var req = {
                method: 'POST',
                url: url,
    ...
    $scope.RemoveTax = function (tax) {
        for (var i = $scope.SelectedTaxList.length - 1; i >= 0; i--) {
            if ($scope.SelectedTaxList[i].OtherAccountId == tax.OtherAccountId) {
